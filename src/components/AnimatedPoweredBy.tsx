...
export function AnimatedPoweredBy({ sources, isMobile = false }: AnimatedPoweredByProps) {
  ...
  useEffect(() => {
    ...
    }, isMobile ? 2500 : 3000) // Faster on mobile
      ...
      >
        ...
        {!isMobile && (
          <span className="text-gray-400 text-xs ml-1">
            ({currentIndex + 1}/{sourceEntries.length})
          </span>
        )}
      </div>
...
export function AnimatedPoweredByFixed({ sources, isMobile = false }: AnimatedPoweredByProps) {
  const [currentSourceIndex, setCurrentSourceIndex] = useState(0)
  const [isVisible, setIsVisible] = useState(true)
